...
library(writexl)
library(ggplot2)
library(pracma)   # provides erf(), used in the Gini section

households_data <- read_csv(DATA_PATH)
...
##
## Uses the cmLN parameters (m, sigma, lambda, epsilon) fitted above,
## rather than separately hardcoded values.
## ----------------------------------------------------------

m_gini       <- fit_cmLN$m
sigma_gini   <- fit_cmLN$sigma
lambda_gini  <- fit_cmLN$lambda
epsilon_gini <- fit_cmLN$epsilon

sigma2_gini <- sigma_gini^2
...
result <- gini(epsilon_gini, m_gini, sigma2_gini, lambda_gini)
